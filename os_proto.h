 ...
 *
 *	As of March 6th, 2000, MEA wrapped the declarations of potentially
 *	dangerous routines with "#if __PART_OF_GUTS__". We decided to leave
 *	these routines in this file to avoid version-skew, but want anybody
 *	using them to think _really_hard_ about why. We also mess around
 *	with the definition sof GREAT_RENAME and OBSOLETE when we are part
 *	of GUTS, as there is even less excuse for GUTS to be ten or more
 *	years behind than there si for game programmers.
 */
...
#ifndef __PART_OF_GUTS__
#ifndef GREAT_RENAME
#define GREAT_RENAME (1)
#endif
#if GREAT_RENAME == 0
#error /* come into the 1990's at least... */
#endif /* GREAT_RENAME */
#endif /* __PART_OF_GUTS__ */
 ...
 */
/* Should be keyed as __PART_OF_GUTS__, but audio is a swamp we do not
 * have time to drain right now MEA 06MAR2000
*/
...
/*	cn_balance(), and cn_charge(), cn_cost(), cn_needed() 
*
*	As of March 6th, 2000, MEA recommends the use of these
*	four routines in place of cn_credits(), cn_chkcoin(),
*	cn_coins(), cn_continues(), cn_dbcoin(), cn_debit(),
*	cn_discount(). The reasons include "basic hygiene" and
*	the need (for the Tournament system) to have multiple
*	(More than two) possible prices for a game. These routines
*	all take a <for_what> parameter indicating the context
*	(type of game) they are being called in. All games will
*	support <for_what> values of zero and one, indicating
*	"new game" and "continuation", respectively. Some games
*	may return sensible values for <for_what> ranging from 2
*	upwards. These are the prices of "special" games, such as
*	tournaments. There are two possible "special return values"
*	for these routines:
*	 -2 (CN_GAME_INVALID) and
*	 -1 (CN_GAME_UNAVAILABLE)
*
*	The difference is that CN_GAME_INVALID is returned for
*	calls with <for_what> set to a value that cannot be valid.
*	(e.g. outside the range of possible values), while
*	CN_GAME_UNAVAILABLE is returned for valid values, that
*	refer to games which are not _currently_ available (e.g.
*	tournaments which have not yet started, or have completed)
*	(For compatibility with previous version, also returns
*	CN_GAME_UNAVAILABLE for calls before cn_init(), which
*	should never happen in a working game.
*
*	cnq_balance(), and cnq_charge(), cnq_cost(), cnq_needed() 
*
*	These routines have been installed to account for the
*	"problem" of tournament ID's being any value from
*	0-0xffffffff. Therefore we cannot double up on the 'for_what'
*	parameter as previously expected. The functions are new
*	to prevent breaking legacy code.
*/

#define CN_GAME_INVALID (-2)
#define CN_GAME_UNAVAILABLE (-1)

/* The following are named constants whose value is _highly_
 * unlikely to change. 
 * The CN_EXTRA_GAME_*
 * ones will provoke CN_GAME_UNAVAILABLE on games which have
 * not defined any EXTRA games (non-tournament, game-specific
 * "special price" games, e.g. the 9- and 18-hole games for
 * Skins-game)
*/

/* Argument 0 for cnq_xxx() can be one of: */
#ifndef CN_LEGACY
#define CN_LEGACY (1)
#endif
#ifndef CN_TOURN
...
/* Argument 1 for cnq_xxx() or argument 0 of cn_xxx() can be one of: */
#ifndef CN_NEW_GAME
#define CN_NEW_GAME (0)
#endif
#ifndef CN_CONTINUE_GAME
#define CN_CONTINUE_GAME (1)
#endif
#ifndef CN_EXTRA_GAME_1
#define CN_EXTRA_GAME_1 (2)
#endif
#ifndef CN_EXTRA_GAME_2
#define CN_EXTRA_GAME_2 (3)
#endif

/*		cn_needed(for_what)
*	Returns the number of additional "coin units" needed to
*	reach the "price" of a game. The type of game is indicated
*	by <for_what>, as detailed above.
*
*  returns:
*   CN_GAME_INVALID	if <for_what> is out-of-range, i.e. impossible.
*   CN_GAME_UNAVAILABLE	if <for_what> selects a currently-unavailable game.
*    0			if <for_what> is sensible, balance is sufficient.
*   >0			if player needs to insert money to play.
*/
extern int cn_needed
PARMS((
    int __for_what	/* See above */
));

/*		cnq_needed(qual_type, for_what)
*	At entry:
*		qual_type - one of CN_LEGACY or CN_TOURN to select type of 'for_what'
*		for_what - if qual_type == CN_LEGACY, one of CN_NEW_GAME, CN_CONTINUE_GAME, etc.
*    			   if qual_type == CN_TOURN, tournament ID
*	At exit:
*		Returns same as cn_needed() as indicated above.
*/
extern int cnq_needed
PARMS((
    int __qual_type,	/* See above */
    int __for_what	/* See above */
));

/* 		cn_balance(for_what)
*  Return the current "balance", We no longer use GreenHills C, but
*  there is no compelling reason to change the return format. See
*  above for description of <for_what> and CN_GAME_* return values.
*
*  Normally returns a signed long which is packed as follows:
...
*   CN_GAME_UNAVAILABLE	if <for_what> selects a currently-unavailable game.
*/
extern long cn_balance
PARMS((
int __for_what 	/* See above */
));

/* 		cnq_balance(qual_type, for_what)
...
*    		   if qual_type == CN_TOURN, tournament ID
*  At exit:
*	returns the same as cn_balance() described above.
*/
extern long cnq_balance
PARMS((
int __qual_type,/* See above */
int __for_what 	/* See above */
));
...
/* 		cn_charge( for_what )
*  	Subtract the appropriate number of "credits", appropriately
*	priced for type of game (see above explanation of <for_what>
*
*	Returns: 1 if successful
*		 0 if insufficient funds
*		CN_GAME_INVALID for impossible <for_what> value.
*		CN_GAME_UNAVAILABLE (also if called before cn_init())
*/
extern int cn_charge
PARMS((
int __for_what 	/* See above */
));

/* 		cnq_charge( qual_type, for_what )
*  	Subtract the appropriate number of "credits", appropriately
*	priced for type of game.
*
*  At entry:
*	qual_type - one of CN_LEGACY or CN_TOURN to select type of 'for_what'
*	for_what - if qual_type == CN_LEGACY, one of CN_NEW_GAME, CN_CONTINUE_GAME, etc.
*    		   if qual_type == CN_TOURN, tournament ID
*  At exit:
*	returns the same as cn_charge() described above.
*/
extern int cnq_charge
PARMS((
int __qual_type, /* See above */
int __for_what 	/* See above */
));
...
/*		cn_clear_coins()
 *	Works with cn_cum_coins(), below, to keep track of cumulative
 *	coins, despite "clearing" of totals by the operator. Should
 *	be called before (or instead of) clearing the CC0..CCX stats
 *	in stats.c.
 */
extern void cn_clear_coins();
...
/* 		cn_cost(for_what)					(coin.c)
*	Return the price (in "coin units") needed to play a game of
*	the type specified by <for_what>. Note that even in free-play,
*	CN_GAME_INVALID and CN_GAME_UNAVAILABLE may be returned.
*
*  returns:
*   CN_GAME_INVALID	if <for_what> is out-of-range, i.e. impossible.
*   CN_GAME_UNAVAILABLE	if <for_what> selects a currently-unavailable game.
*    0			if <for_what> is sensible, free-play
*   >0			Cost of this type of game ("unit coins")
*/
extern int cn_cost
PARMS((
    int __for_what	/* See above */
...
/* 		cnq_cost(qual_type, for_what)				(coin.c)
*	Return the price (in "coin units") needed to play a game of
*	the type specified by <for_what>. Note that even in free-play,
*	CN_GAME_INVALID and CN_GAME_UNAVAILABLE may be returned.
*
...
*    		   if qual_type == CN_TOURN, tournament ID
*  At exit:
*	returns the same as cn_cost() described above.
*/
extern int cnq_cost
PARMS((
    int __qual_type,	/* See above */
...
/*		cn_cum_coins( service, new, new_service )
 *	Returns _cumulative_ count of coins/service-switch "coins"
 *	since the EEPROM/NVRAM was initialized. This include both
 *	the "current" counts and any counts that were "cleared"
 *	by operator action. If you just want the total, all three
 *	parameters may be 0: cn_cum_coins(0, 0, 0).
 *	If you want a breakdonw by new/old, service/real, specify
 *	pointers for each portion you want separately reported.
 *	Any portion of the cumulative coin-count that is not stored
 *	via a supplied pointer is added to the return value.
 *	A game must define the stats CUM_COINS and CUM_SERVICE to
 *	make this work, and must call cn_clear_coins() either before
 *	or instead of clearing the CC0..CCX stats (in stats.c)
 */

extern long cn_cum_coins
PARMS((
    unsigned long *_service, 	/* If !=0, report total/old service credits here */
    unsigned long *_new_coins, 	/* if !=0, report new coins here */
    unsigned long *_new_svc	/* if !=0, report new service credits here */
));

#if __PART_OF_GUTS__ && OBSOLETE /* Not bloody likely */

/* 		cn_cvt_crdt(value)				(coin.c)
*	Convert coin-inputs to credits. Called with unsigned long <value>
*	containing 2-bit switch bits. Actually, they are counter "deltas",
*	i.e. differences between old and new values for each mech.
...
#endif /* __PART_OF_GUTS__ && OBSOLETE */ 
 ...
 *
 *	Deprecated in favor of cn_needed().
 */
extern int cn_next
PARMS((
 ...
 *	cn_init()
 *
 *	Deprecated in favor of cn_cost().
 */
extern int cn_price();

#if 1 || __PART_OF_GUTS__
/*		cn_register_price(for_what, how_much, use_bonus)
*	Sets the price for a certain type of game. <for_what>
*	may be a tournament ID, or one of CN_NEW_GAME or
*	CN_CONTINUE_GAME. We are only _hoping_ this will work,
*	as Agames/MGWest has no control over these things.
*	<how_much> is specified in "coin units", although this,
*	too may be subject to the whim of someone who has not
*	fully considered the ramifications. <use_bonus> is
*	zero (or FALSE, for Pascalites) if bonus units cannot
*	be applied to this type of game, non-zero (TRUE) if
*	bonus units can be applied.
*
*	As currently implemented, this function will _not_
*	set the price of a NEW or CONTINUE game, and will
*	return CN_GAME_INVALID if this is attempted. This
*	is a policy decision, and may be revised.
*
*	returns:
*	CN_GAME_INVALID	if for some reason a price cannot
*		be registered.
*	0	for success.
*/
extern int cn_register_price
PARMS((
    int __for_what,	/* as described above */
    unsigned int __how_much,	/* in "coin units" */
    int __use_bonus	/* as described above */
));

/*		cnq_register_price(qual_type, for_what, how_much, use_bonus)
* Sets the price for a certain type of game. 
* At entry:
*	qual_type - one of CN_LEGACY or CN_TOURN
*	for_what - if qual_type == CN_LEGACY, one of CN_NEW_GAME,
*    			CN_CONTINUE_GAME, etc.
*		   if qual_type == CN_TOURN, tournament ID
*	<how_much> - number of "coin units" for this game
*	<use_bonus> - non-zero if bonus units can be applied to this type of game
* At exit:
*	returns:
*	CN_GAME_INVALID	if for some reason a price cannot
*		be registered.
...
*
*/
extern int cnq_register_price
PARMS((
    int __qual_type,	/* as described above */
    int __for_what,	/* as described above */
    unsigned int __how_much,	/* in "coin units" */
    int __use_bonus	/* as described above */
));

/*		cnq_unregister_price(qual_type, for_what)
* Removes the "registration" of the price for a certain type of game.
* 
* At entry:
*	qual_type - one of CN_LEGACY or CN_TOURN
*	for_what - if qual_type == CN_LEGACY, one of CN_NEW_GAME,
...
*	0	for success.
*
*	As currently implemented, this function will _not_ alter
*	or remove the price of a NEW or CONTINUE game, and will
*	return CN_GAME_INVALID if this is attempted. This
*	is a policy decision, and may be revised.
*
*/
extern int cnq_unregister_price
PARMS((
    int __qual_type, 		/* as described above */
    int __for_what		/* as described above */
));
...
/*		cn_select( play_mask )
*	Only available in mp_coin.c, for games with multiple "purses".
*
*	Sets up suppression-mask to not suppress (i.e. allow to be
*	used) those mechs which any of the players represented in
*	play_mask can access. D0 of play_mask is one if the first
*	player's mechs are to be included, D1 if the second player's
*	mechs are to be included, etc.
*
*	Returns previous value, so it can be saved/restored
*
*/
extern int cn_select
PARMS((
unsigned int play_mask
));
...
/*		cn_uses_bonus( for_what )
*	Indicates whether the type of game indicated by <for_what>
*	can use bonus coins. Tournament-type games will likely not
*	do so. Solo games may. This is primarily of interest only
*	to the coin routines themselves.
*
*   returns:
*	zero if a given game type cannot use bonus coins
*	1	if a given game type can use bonus coins
*
*/
extern int cn_uses_bonus
PARMS((
    int __for_what	/* as described above */
));

/*		cnq_uses_bonus( qual_type, for_what )
*	Indicates whether the type of game indicated by <for_what>
*	can use bonus coins. Tournament-type games will likely not
*	do so. Solo games may. This is primarily of interest only
*	to the coin routines themselves.
...
struct ctl_arpt {
    unsigned long switches;	/* switch mask */
    int initial;		/* Number of fields before first auto-closure */
    int delay;			/* Number of fields between autorepeats */
};

/*		ctl_autorepeat_plyr()				(ctl_sw.c)
 * Set one or more control switches to fake edges periodically.
 * At entry:
 *	__player - index to player controls (0 to N_PLAYERS-1)
 *	__new - pointer to struct containing new values (if 0, changes nothing)
 *	__old - pointer to struct to get old values (if 0, saves nothing)
 * At exit:
 *	returns 0 if success or 1 if failure (__player out of range).
 *	Fills fields in 'old', if provided, with previous values.
 *	Sets new values from 'new', if provided, else makes no changes.
 */
extern int ctl_autorepeat_plyr
PARMS((
    int __player,		/* Which set of player controls to apply */
    struct ctl_arpt *__new,	/* pointer to new values */
    struct ctl_arpt *__old	/* pointer to old values */
));
...
/*		ctl_read_plyr(player, edges)				(ctl_sw.c)
 *	Returns the state of the game controls as switches. A binary 1
 *	corresponds to an asserted switch (or newly asserted switch).
 *
 *	If the bit corresponding to a switch is set in <edges>, it is
 ...
 *	do exist.
 */
extern unsigned long ctl_read_plyr
PARMS((
    int player,			/* Index of player (0-n where n=max player-1) */
    unsigned long __edges	/* Mask of which switches show only edges */
));

#ifndef TRKBALL_POS_HISTORY
#define TRKBALL_POS_HISTORY (4)
#endif
struct trkball_str {
    int x_position[TRKBALL_POS_HISTORY]; /* current trackball position (0th entry) + last n samples */
    int y_position[TRKBALL_POS_HISTORY];
    int xerrs;				/* sample error counter (from h/w) */
    int yerrs;
};

/*		ctl_read_tb(which, where)
 *	Returns the position and velocity of the selected trackball.
 */
extern unsigned long ctl_read_tb
PARMS((
    int which,			/* Index of trackball (0-n where n=max trackball-1) */
    struct trkball_str *ptr	/* pointer to place to put results */
));

struct trkball_raw_str {
    int abs_x_pos;		/* absolute X position from trackball register */
    int abs_y_pos;		/* absolute Y position from trackball register */
#define TRKBALL_X_QUAD_A (1<<0)	/* where the quadrature bits are */
#define TRKBALL_X_QUAD_B (1<<1)
#define TRKBALL_Y_QUAD_A (1<<2)
#define TRKBALL_Y_QUAD_B (1<<3)
    U8  pins;			/* trackball quadrature signals (raw h/w) */
    U8  filter;			/* contents of the filter register */
};

/*		ctl_read_rawtb(which, where, filter)
 *	Returns the registers from the trackball h/w. Also sets the
 *	filter mode if the h/w supports it.
 */
extern unsigned long ctl_read_rawtb
PARMS((
    int which,			/* Index of trackball (0-n where n=max trackball-1) */
    struct trkball_raw_str *ptr /* pointer to place to put results */
));

/*		ctl_mod_tb( filter )
 *	Set the trackball filter value. Returns previous value.
 *	If filter == -1, resets all the trackball position registers
 *	both in s/w and in h/w and doesn't change the filter register.
 */
extern int ctl_mod_tb
PARMS((
    int filter			/* New filter value (if -1, resets all registers instead) */
));
 ...
 *	It should _never_ be modified by any other code. This means
 *	you. It will take over a year of continual power on to
 *	"wrap negative". When properly used (subtract some epoch time
 *	from the current value) it should pose no problems in timing
 *	events of less than two years duration. That's a _long_
 *	game time. Since some programmers need a bit more hand_holding,
 ...
 *
 *	Better aging is done automatically if POT_AUTO_AGE is defined
 *	to be non-zero.
 */
extern void pot_age
PARMS((
...
/*		st_poll()
 * 	WMS games use a momentary-contact button to enter selftest,
 *	and we are supposed to do so from "within" the game. On
 *	such systems as this really is possible, the game should
 *	call st_poll() periodically (e.g. in main event loop).
 *	st_poll() will watch the TEST switch, debounce it, and run
 *	selftest "nested" as a subroutine. It returns 0 if it did
 *	not run selfteste, otherwise returns the number of frames
 *	spent in selftest. The environment variables PRE_ST and
 *	POST_ST can declare "int functions(void)" to be called
 *	before and after the main screen of selftest. These "hooks"
 *	can be used to, e.g. save and restore the palettes, audio
 *	system, etc.
 */
extern unsigned long st_poll();
...
/*
 * The shuffle functions will return the numbers from 1-n in a
 * psuedo random (but always repeating) order without repeating
 * any given number before having returned all the others.
 * One needs to call shuffle_reset() with details before making
 * subsequent shuffle_next() calls. That is to say, if one calls
 * shuffle_reset( ptr, 1, 65535), then each call to shuffle_next()
 * will return a number in the range of 1 to 65535 reporting each
 * number in the range no more than once in 65535 calls.
 */

typedef struct shuffle_bits {
    unsigned long element;
    unsigned long mask;
} ShuffleBits;

/*
 * shuffle_reset( ShuffleBits *sb, unsigned long element, unsigned long largest) (random.c)
 *
 * At entry:
 *	sb - pointer to struct into which details about subsequent calls
 *		to shuffle_next will be stored.
 *	element - starting element (1 to n)
 *	largest - highest number for shuffle_next to return (1-(2^32-1))
 * At exit:
 *	returns nothing. Will have set the members in sb appropriately.
 */
extern void shuffle_reset
PARMS(( ShuffleBits *sb, unsigned long element, unsigned long largest ));

/*
 * shuffle_next( ShuffleBits *sb )		(random.c and here)
 *
 * At entry:
 *	sb - pointer to struct previously used in a shuffle_reset() call.
 * At exit:
 *	returns next number in sequence. Will have updated member
 *	'element' in the sb struct.
 * NOTE: This function can be made inline by defining SHUFFLE_NEXT_INLINE
 * in config.h.
 */
#define SHUFFLE_NEXT_FUNCTION 						\
unsigned long shuffle_next( ShuffleBits *sb ) {					\
    if (sb->element&1) sb->element = (sb->element >>1)^sb->mask;	\
    else sb->element = (sb->element >>1);				\
    return sb->element;							\
}

#if !defined(SHUFFLE_NEXT_INLINE) || !SHUFFLE_NEXT_INLINE
extern unsigned long shuffle_next PARMS(( ShuffleBits *sb ));
#else
__inline__ SHUFFLE_NEXT_FUNCTION
#endif
...
PARMS((
    ...
    int		__stamp,/* stamp number (AN_*) to output		*/
...
/*		vid_vis_pix( void )
 *	Returns resolution packed into 32 bits.
 *	Vertical resolution in the lower 16 bits.
 *	Horizontal resolution shifted left by 16 bits.
 */
extern unsigned long vid_vis_pix( void );
